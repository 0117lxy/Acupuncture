...
public class ClickToAcuDesription : MonoBehaviour
{
    ...
    TipForAcuDescirption tipForAcuDescirption;//第三关点击穴位的详细提示。
    ...
    void Start()
    {
        ...
        tipForAcuDescirption = GameObject.Find("ClickTip").GetComponent<TipForAcuDescirption>();
    ...
    void ButtonClick(uint index, GameObject anchor)
    {
        ...
        if (index != tipForAcuDescirption.clickQueue[tipForAcuDescirption.clickNum])
        {
            if (lifeNumberChange.theHeartNumber != 0)
            {
                lifeNumberChange.theHeartNumber--;
